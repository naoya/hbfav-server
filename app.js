...
(function() {
  var $, Timeline, _, app, express, jsdom, prettyDate, request, rss2timeline, toEpoch, window, xml2js;
  ...
  Timeline = (function() {
    function Timeline(feed) {
      ...
      this.bookmarks = _.map(feed.item, function(item) {
  ...
  Timeline.Bookmark = (function() {
    function Bookmark(item) {
      ...
      this.comment = (ref = item.description) != null ? ref : "";
      ...
      if (item['content:encoded']) {
        ...
        this.description = element.find('p').text();
        if (/class="entry-image"/.test(item['content:encoded'])) {
          this.thumbnail_url = element.find('.entry-image').attr('src');
        }
      }
    }
  ...
  app.get("/:id/bookmark", function(req, res) {
    var offset, ref, url;
    offset = (ref = req.param('of')) != null ? ref : 0;
